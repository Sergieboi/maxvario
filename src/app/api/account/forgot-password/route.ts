...
export async function POST(request: NextRequest) {
  ...
  const res = await wpRequest(`${process.env.NEXT_PUBLIC_MAXVARIO_API}/forgot-password`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(body),
  });
  if (!res) return NextResponse.json({ success: false, messages: ["Server error"] }, { status: 500 });
